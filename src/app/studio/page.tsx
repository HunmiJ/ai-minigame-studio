...
export default function StudioPage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      if (!saved) { setNotice("未找到该作品，已保留当前 Demo。"); return; }
      setGame(saved.gameSpec); setPrompt(saved.sourcePrompt); setStyle(saved.styleId);
      setMode(saved.source === "ai" ? "ai" : "demo"); setSource(saved.source === "template" ? saved.gameSpec.genre : "custom");
      setPreviewSource(saved.source);
      setProjectId(saved.id); setSaveStatus("saved"); setResetToken((token) => token + 1);
      setNotice("已打开本地作品，可以继续试玩或修改。");
    }, 0);
  ...
  const generate = async () => {
    ...
    if (cleanedPrompt.length < 10) { setError("请至少输入 10 个字符的游戏创意后再生成。"); return; }
    ...
    try {
      ...
      if (!response.ok || !body.game) {
        const code = body.error?.code; setServiceStatus(code === "not_configured" ? "missing" : "connected");
        setError(getGenerationFailureMessage(code, body.error?.message)); if (code === "not_configured") setMode("demo"); return;
      }
      setGame(body.game); setMode("ai"); setPreviewSource("ai"); setProjectId(undefined); markUnsaved(); setServiceStatus("connected");
      setResetToken((token) => token + 1); setNotice("AI 游戏已生成，可以开始试玩。尚未保存到作品库。");
    } catch { setServiceStatus("connected"); setError("DeepSeek 已连接，但本次生成失败；当前 Demo 已保留。请检查网络后重新尝试。"); }
    finally { setIsGenerating(false); }
  };
  const changeStyle = (nextStyle: GameStyleId) => { setStyle(nextStyle); markUnsaved(); if (mode === "demo") { setGame(createStyledDemoGame(nextStyle, game.genre)); setResetToken((token) => token + 1); setNotice(`Demo 游戏已切换为“${demoGameStyles[nextStyle].label}”风格。`); } };
  const selectTemplate = (genre: GameSpec["genre"]) => { setPrompt(prompts[genre]); setSource(genre); setGame(createStyledDemoGame(style, genre)); setMode("demo"); setPreviewSource("template"); setProjectId(undefined); markUnsaved(); setResetToken((token) => token + 1); setError(""); setNotice(`已载入${genre === "dodge" ? "星际闪避" : genre === "collect" ? "接金币" : "迷你迷宫"} Demo。`); };
  const editPrompt = (value: string) => { setPrompt(value); setSource("custom"); markUnsaved(); };
  const saveProject = () => { setSaveStatus("saving"); try { const saved = saveGame(window.localStorage, buildDraft(), projectId); setProjectId(saved.id); setSaveStatus("saved"); setNotice("作品已保存到当前浏览器的作品库。"); } catch { setSaveStatus("failed"); setError("本地作品保存失败，请检查浏览器存储空间后重试。"); } };
  const exportProject = () => { try { const saved = createSavedGame(buildDraft()); const blob = new Blob([exportSavedGameJson(saved)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = createExportFilename(game.title); link.click(); URL.revokeObjectURL(url); setNotice("JSON 作品文件已导出。"); } catch { setError("作品导出失败，请稍后重试。"); } };
  const saveText = saveStatus === "saving" ? "正在保存" : saveStatus === "saved" ? "已保存到作品库" : saveStatus === "failed" ? "保存失败" : "未保存";

  return <main className="studio-shell" style={{ "--studio-accent": game.theme.accentColor } as React.CSSProperties}><div className="studio-topbar"><Link href="/" className="back-link">← 返回首页</Link><span className="studio-brand"><i>✦</i> AI MiniGame Studio</span><div className="studio-top-actions"><Link href="/gallery" className="back-link">作品库</Link><button type="button" className="studio-action" onClick={exportProject}>导出 JSON</button><button type="button" className="studio-action primary" onClick={saveProject} disabled={saveStatus === "saving"}>{saveStatus === "saving" ? "保存中…" : "保存作品"}</button><span className={`save-state ${saveStatus}`}>● {saveText}</span></div></div><div className="studio-layout"><PromptPanel prompt={prompt} source={source} genre={game.genre} visualTheme={game.visualTheme} selectedStyle={style} onPromptChange={editPrompt} onTemplateSelect={selectTemplate} onStyleChange={changeStyle} onGenerate={generate} notice={notice} error={error} isGenerating={isGenerating} mode={mode} serviceStatus={serviceStatus} /><GamePreview game={game} resetToken={resetToken} source={previewSource} /></div></main>;
}
