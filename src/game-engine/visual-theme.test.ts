...
import { normalizeVisualTheme, renderVisualTheme } from "./visual-theme";
...
describe("visual theme normalization", () => {
  ...
  it("keeps ocean and selects the ocean palette", () => { const { ctx, gradient } = context(); renderVisualTheme(ctx, "ocean", 960, 540); expect(normalizeVisualTheme("ocean")).toBe("ocean"); expect(gradient.addColorStop).toHaveBeenCalledWith(0, "#063d71"); });
