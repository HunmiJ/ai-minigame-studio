...
const themeMatchers: Array<[VisualTheme, RegExp]> = [
  ["ocean", /海底|海洋|深海|珍珠|水母|珊瑚/],
  ["lava", /熔岩|火山|岩浆|火焰/],
  ["ice", /冰雪|冰川|雪地|极地|寒冰|冬季/],
  ["forest", /森林|树林|丛林|自然|树木|苔藓|草地/],
  ["neon", /霓虹|赛博朋克|未来都市/],
  ["desert", /沙漠|沙丘|荒漠|绿洲/],
  ["space", /太空|星际|宇宙|银河|星空|陨石/],
];
...
function findKeywords(prompt: string, candidates: string[]) { return candidates.filter((candidate) => prompt.includes(candidate)); }

export function extractGameIntent(prompt: string): GameIntent {
  const genre = /2048|合并数字|滑动数字|数字方块/.test(prompt) ? "2048"
    : /贪吃蛇|蛇吃食物|身体变长|snake/i.test(prompt) ? "snake"
      : /迷宫|出口|探索/.test(prompt) ? "maze"
        : /接金币|收集|接住|珍珠|金币|宝石|星星/.test(prompt) ? "collect"
          : /躲避|闪避|生存|陨石/.test(prompt) ? "dodge" : undefined;
  ...
  return {
    genre, visualTheme, duration, lives, targetCount, targetTile, difficulty,
    collectibleKeywords: findKeywords(prompt, collectKinds),
    hazardKeywords: findKeywords(prompt, hazardKinds),
    scenarioKeywords: themeMatchers.filter(([, matcher]) => matcher.test(prompt)).map(([theme]) => theme),
    unsupportedFeatures: [],
  };
}
...
export function evaluateRequirementMatch(intent: GameIntent, game: GameSpec): RequirementMatch {
  ...
  return { met, adapted, unsupported, primarySatisfied: genreOk && themeOk && durationOk && livesOk && targetOk && tileOk && unsupported.length === 0, theme: intent.visualTheme ?? game.visualTheme };
}
