...
export function extractGameIntent(prompt: string): GameIntent {
  ...
  return { genre, visualTheme, duration, lives, targetCount, difficulty, collectibleKeywords: findKeywords(prompt, collectKinds), hazardKeywords: findKeywords(prompt, hazardKinds), scenarioKeywords: themeMatchers.filter(([, matcher]) => matcher.test(prompt)).map(([theme]) => theme), unsupportedFeatures };
}
...
export function evaluateRequirementMatch(intent: GameIntent, game: GameSpec): RequirementMatch {
  ...
  return { met, adapted, unsupported, primarySatisfied: genreOk && themeOk && durationOk && livesOk && targetOk && unsupported.length === 0 };
}
